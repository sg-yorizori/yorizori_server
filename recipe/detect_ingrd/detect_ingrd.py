...
class Dectect_Ingrd:
    ...
    def detect(self, img_path): #return result, np_result
        img = Image.open(img_path)
        result = self.model(img)
        return result, result.pandas().xyxy[0].to_numpy()

if __name__ == "__main__":
    detection = Dectect_Ingrd() #Dectect_Ingrd('best')
    result, np_result = detection.detect("E:/test.jpg") #for test

    # result.print()
    # result.save()

    # xmin / ymin / xmax / ymax / confidence / class / name
    print(np_result[0][-1]) #numpy format
